...
def get_candidates(crossword):
    ...
    for idx, key in enumerate(keys):
        ...
        completions = Model.generate_unique_completions(clue, length, num_completions=10)
...
def solve(filepath, index=None):
    ...
    #             row_letters.append('#')
    #         else:
    #             row_letters.append(truth_solution[(i, j)])
    #     print(' '.join(row_letters))
    ...
    # Generate & save true grid
    fig2, _ = plot_grid(truth_solution, crossword, n, title="True Grid")
    fname2 = f"true_{index}.png" if index is not None else "true.png"
    fig2.savefig(os.path.join(plots_dir, fname2))
    plt.close(fig2)
